...
#include <hal/video/amiga/iff/iff_events.hh>
...
namespace formats::iff
{
    namespace detail
    {
        ...
        std::tuple<chunk_type, std::size_t> read_chunk_type(std::istream& is)
        {
            ...
            {
                return std::make_tuple(chunk_type{neutrino::utils::byte_order::from_big_endian(dword)}, sizeof(dword));
            }
            return std::make_tuple(chunk_type{neutrino::utils::byte_order::from_little_endian(dword)}, sizeof(dword));
        }
        // -------------------------------------------------------------------------------------------
        template<typename IffTraits>
        std::tuple<std::size_t, std::size_t> read_chunk_length(std::istream& is)
        {
            ...
            {
                return std::make_tuple(IffTraits::correct_size(neutrino::utils::byte_order::from_big_endian(dword)), sizeof(dword));
            }
            return std::make_tuple(IffTraits::correct_size(neutrino::utils::byte_order::from_little_endian(dword)), sizeof(dword));
        }
        // -------------------------------------------------------------------------------------------
        template<typename IffTraits>
