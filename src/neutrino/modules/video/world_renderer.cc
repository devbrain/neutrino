...
#include <neutrino/modules/video/renderer_state.hh>
        ...
        for (const auto& layer : *m_model) {
            std::visit(
                bsw::overload(
                    ...
                    [this, delta_time](const tiled::objects_layer& ol) {
                        auto& obj_layer = const_cast <tiled::objects_layer&>(ol);
                        draw_layer(obj_layer, delta_time);
                    }
                ),
                layer
            );
