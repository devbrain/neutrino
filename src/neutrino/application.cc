...
namespace neutrino {
	...
	void application::init(unsigned w, unsigned h, bool fullscreen, int desired_fps) {
		...
		m_main_window = fullscreen ? sdl::window(w, h, sdl::window::flags::FULL_SCREEN_DESKTOP) : sdl::window(w, h);
	...
	void application::run() {
		...
		while (!need_to_quit()) {
			...
			start = s; {
				auto top_scene = m_scene_manager.top();
				ENFORCE(top_scene);
				try {
					...
					while (SDL_PollEvent(&sdl_event)) {
						bool do_quit = false;
						switch ((unsigned int)sdl_event.window.event) {
							case SDL_QUIT:
								do_quit = true;
								break;
