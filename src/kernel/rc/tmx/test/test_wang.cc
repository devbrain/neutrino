...
static void test_wang (const map& the_map) {
  REQUIRE(the_map.orientation () == neutrino::kernel::orientation_t::STAGGERED);
  REQUIRE(the_map.render_order () == neutrino::kernel::render_order_t::RIGHT_DOWN);
  REQUIRE(the_map.infinite ());
  REQUIRE(the_map.stagger_axis () == neutrino::kernel::stagger_axis_t::Y);
  REQUIRE(the_map.stagger_index () == neutrino::kernel::stagger_index_t::ODD);
