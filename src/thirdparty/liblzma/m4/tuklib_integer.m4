...
AC_MSG_CHECKING([if __builtin_bswap16/32/64
are supported
])
...
if
test x$ac_cv_header_byteswap_h = xyes;
then
    m4_foreach (
[FUNC], [bswap_16,bswap_32,bswap_64], [
AC_MSG_CHECKING([if
FUNC is
available])
...
if test "x$enable_unaligned_access" =
xauto;
then
...
then
    AC_DEFINE (
[TUKLIB_USE_UNSAFE_TYPE_PUNNING], [1], [
Define to
1
to use
unsafe type
