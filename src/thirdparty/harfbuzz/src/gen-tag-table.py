...
class LanguageTag(object):
    ...
    def __init__(self, tag):
        ...
        if self.grandfathered:
            ...
            self.region = self._find_first(lambda s: len(s) == 2 and s[0] > '9' or len(s) == 3 and s[0] <= '9',
                                           self.subtags[1:])
...
def verify_disambiguation_dict():
    ...
    for ot_tag, bcp_47_tags in ot.to_bcp_47.items():
        if ot_tag == DEFAULT_LANGUAGE_SYSTEM:
            ...
            primary_tags = list(
                t for t in bcp_47_tags if t not in bcp_47.grandfathered and ot.from_bcp_47.get(t)[0] == ot_tag)
        if len(primary_tags) == 1:
            ...
            if different_bcp_47_tags and disambiguation[ot_tag] == different_bcp_47_tags[0] and '-' not in \
                    disambiguation[ot_tag]:
