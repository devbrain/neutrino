    ...
    if (matchLength > bestLength) {
      ...
      if ((4 * (int) (matchLength - bestLength))
        ...
        DEBUGLOG(9, "ZSTD_DUBT_findBetterDictMatch(%u) : found better match length %u -> %u and offsetCode %u -> %u (dictMatchIndex %u, matchIndex %u)",
                 ...
                 ZSTD_REP_MOVE + curr - matchIndex, dictMatchIndex, matchIndex);
...
ZSTD_row_prefetch (U32 const *hashTable, U16 const *tagTable, U32 const relRow, U32 const rowLog) {
  ...
  assert(ZSTD_isAligned (
      tagTable + relRow, (size_t) 1 << rowLog)); /* prefetched tagRow sits on a multiple of 32 or 64 bytes */
}
...
FORCE_INLINE_TEMPLATE void ZSTD_row_update_internal (ZSTD_matchState_t *ms, const BYTE *ip,
  ...
  for (; idx < target; ++idx) {
    U32 const hash = useCache
                     ? ZSTD_row_nextCachedHash (ms->hashCache, hashTable, tagTable, base, idx, hashLog, rowLog, mls)
...
FORCE_INLINE_TEMPLATE size_t ZSTD_RowFindBestMatch_selectMLS (
  ...
  switch (ms->cParams.minMatch) {
    default : /* includes case 3 */
    case 4 :
      return ZSTD_RowFindBestMatch_generic (ms, ip, iLimit, offsetPtr, 4, dictMode, rowLog);
    case 5 :
      return ZSTD_RowFindBestMatch_generic (ms, ip, iLimit, offsetPtr, 5, dictMode, rowLog);
    case 7 :
    case 6 :
      return ZSTD_RowFindBestMatch_generic (ms, ip, iLimit, offsetPtr, 6, dictMode, rowLog);
  }
}
...
FORCE_INLINE_TEMPLATE size_t ZSTD_RowFindBestMatch_selectRowLog (
  ...
  switch (cappedSearchLog) {
    default :
    case 4 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_noDict, offsetPtr, 4);
    case 5 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_noDict, offsetPtr, 5);
  }
}
...
FORCE_INLINE_TEMPLATE size_t ZSTD_RowFindBestMatch_dictMatchState_selectRowLog (
  ...
  switch (cappedSearchLog) {
    default :
    case 4 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_dictMatchState, offsetPtr, 4);
    case 5 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_dictMatchState, offsetPtr, 5);
  }
}
...
FORCE_INLINE_TEMPLATE size_t ZSTD_RowFindBestMatch_dedicatedDictSearch_selectRowLog (
  ...
  switch (cappedSearchLog) {
    default :
    case 4 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_dedicatedDictSearch, offsetPtr, 4);
    case 5 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_dedicatedDictSearch, offsetPtr, 5);
  }
}
...
FORCE_INLINE_TEMPLATE size_t ZSTD_RowFindBestMatch_extDict_selectRowLog (
  ...
  switch (cappedSearchLog) {
    default :
    case 4 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_extDict, offsetPtr, 4);
    case 5 :
      return ZSTD_RowFindBestMatch_selectMLS (ms, ip, iLimit, ZSTD_extDict, offsetPtr, 5);
  }
}
