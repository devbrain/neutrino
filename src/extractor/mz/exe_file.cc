...
namespace formats::explode::mz
{
            ...
            : m_file(file)
    {
        ...
        {
            m_header[i] = neutrino::utils::byte_order::from_little_endian(m_header[i]);
        }
    ...
    void full_exe_file::write(io::output& out) const
    {
        ...
        for (int i = 0; i < MAX_HEADER_VAL; i++)
        {
            new_header[i] = neutrino::utils::byte_order::to_little_endian(m_header[i]);
        }
        ...
        if (!m_rellocs.empty())
        {
            ...
            {
                const uint16_t rel = neutrino::utils::byte_order::to_little_endian(m_rellocs[i].rel);
                const uint16_t seg = neutrino::utils::byte_order::to_little_endian(m_rellocs[i].seg);
                new_rel[2 * i] = rel;
                new_rel[2 * i + 1] = seg;
            }
            r.words = &new_rel[0];
