...
void add_object(const cell& c, ecs_registry& reg, int x, int y) {
	...
	neutrino::ecs::physics::body phys{};
	...
	if (c.has_property(CC_CELL_TYPE)) {
		...
		if (cell_type == CC_CELL_PLAYER_START) {
			neutrino::ecs::physics::dynamic_object dyn_obj;
			dyn_obj.boundary = neutrino::ecs::physics::rectangle (TILE_W, TILE_H);

			auto eid = builder
				.with_component<neutrino::ecs::physics::body>(phys)
				.with_component<neutrino::ecs::physics::dynamic_object>(dyn_obj)
	...
	if (!c.is_empty()) {
		...
		(void) builder
			.with_component<neutrino::ecs::physics::body>(phys)
			.with_component<neutrino::ecs::physics::static_object>()
