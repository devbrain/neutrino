...
namespace neutrino {
	...
	struct NEUTRINO_EXPORT sprite_bank {
		...
		sprite_bank()
			: current(0), time_in_current_frame(0) {}
		...
		void next(const std::chrono::milliseconds& time_for_frame) {
			if (time_in_current_frame > time_for_frame) {
				time_in_current_frame = std::chrono::milliseconds{0};
				next();
			}
		}
		...
		std::size_t current;
		std::chrono::milliseconds time_in_current_frame;
