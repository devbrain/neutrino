...
namespace neutrino::utils {
	namespace detail {
		NEUTRINO_EXPORT nlohmann::json load_from_json(const std::string& input);
		NEUTRINO_EXPORT nlohmann::json load_from_ini(const std::string& input);
		NEUTRINO_EXPORT nlohmann::json load_from_yaml(const std::string& input);
	...
	class NEUTRINO_EXPORT config_parser {
		public:
			enum kind_t {
				INI,
				JSON,
		...
		private:
			...
			static void parse(std::vector <std::map <std::string, std::any>>& out, nlohmann::json const & map);
		...
		return std::visit(
			overload(
				...
				[](const nlohmann::json& map) {
					return config_parser::parse <Config>(map);
				}
			),
			res
		);
	...
	template<class Config>
	void config_parser::parse(Config& out, const std::string& input, kind_t kind) {
		const auto res = parser_dispatch(input, kind);
		if (const auto* e = std::get_if<std::string>(&res)) {
			RAISE_EX("Failed to parse ",
							 kind == YAML ? "YAML" : (kind == JSON) ? "JSON" : "INI",
							 " Error: ", *e);
		}
		if (const auto* mp = std::get_if<nlohmann::json>(&res)) {
			parse(out, *mp);
		}
	}
		...
		if constexpr (std::is_fundamental_v <field_t>) {
			...
			if constexpr (detail::is_container <field_t>::value) {
				...
				if constexpr (std::is_same_v <field_t, std::map <std::string, std::any>>) {
					...
					RAISE_EX("Only fundamental types, map<string,any> or vector/list are supported",
					         type_name_v<field_t>, " is supplied");
				}
			}
		}
	}
